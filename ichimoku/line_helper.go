...
func (n *lineHelper) isAboveLine(point Point, line []Point) EPointLocation {
	...
	checkAboveOrBelow := func(p Point, line_p_a Point, line_p_b Point) EPointLocation {
		...
		} else if v.GreaterThan(decimal.Zero) {
...
func (n *lineHelper) positionPointInline(point Point, line []Point) decimal.Decimal {

	if len(line) < 2 {
		return decimal.Zero
	}

	checkAboveOrBelow := func(p Point, line_p_a Point, line_p_b Point) decimal.Decimal {
		//var d = (_p_x- _x1) *(_y2-_y1)-(_p_y-_y1)*(_x2-_x1)
...
func (o *lineHelper) GetCollisionDetection(a Point, b Point, c Point, d Point) (EInterSectionStatus, decimal.Decimal) {
	...
	if denominator.Cmp(decimal.Zero) == 0 {
		return EInterSectionStatus_NAN, decimal.Zero
	}
	r := numerator1.Div(denominator)
	s := numerator2.Div(denominator)
	one := decimal.NewFromInt(1)
	if (r.GreaterThanOrEqual(decimal.Zero) &&
		r.LessThanOrEqual(one)) &&
		(s.GreaterThanOrEqual(decimal.Zero) &&
			s.LessThanOrEqual(one)) {
...
func (o *lineHelper) GetCollisionWithLine(price_point Point, line_clouds []Point) (EPointLocation, error) {
	...
	for i := 1; i < len_line_clouds; i++ {
		...
		if c.GreaterThan(decimal.Zero) { //below
			below++
		} else {
			above++
		}
	...
	v := sum.Div(decimal.NewFromInt(int64(len_line_clouds)))
...
func (o *lineHelper) getLineEquation(p1 Point, p2 Point) *Equation {
	...
	y := p1.Y.Sub(p2.Y)
	eq.Slope = y.Div(x)
	eq.Intercept = decimal.NewFromInt(-1).Mul(eq.Slope).Mul(p1.X).Add(p1.Y)
	return &eq
}

func (o *lineHelper) get_intersection_point(line1_pointA Point, line1_pointB Point, line2_pointA Point, line2_pointB Point) decimal.Decimal {
