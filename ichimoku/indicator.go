...
type IIchimokuIndicator interface {
	...
	// one call per asset
	MakeIchimokuInPast(bars *techan.TimeSeries, numberOfRead int) error

	//one call per asset
	PreAnalyseIchimoku(data []*IchimokuStatus) (*IchimokuStatus, error)

	//Analyse Trigger Cross tenken & kijon sen
	AnalyseTriggerCross(previous *IchimokuStatus, bars_only_25_bars_latest []*techan.Candle) (*IchimokuStatus, error)
...
func (xx *IchimokuIndicator) loadbars(from int, to int) []*techan.Candle {
	if len(xx.series.Candles) == 0 {
		return nil
	}
	if from < 0 {
		from = 0
	}
	if to > len(xx.series.Candles) {
		to = len(xx.series.Candles)
	}

	return xx.series.Candles[from:to]
...
func (o *IchimokuIndicator) MakeIchimokuInPast(ts *techan.TimeSeries, numberOfRead int) error {

	if len(ts.Candles) == 0 {
		return ErrDataNotFill
	}

	if len(ts.Candles) < numberOfRead || len(ts.Candles) < 52 {
		return ErrNotEnoughData
	}

	fmt.Printf("Calc ichi from Last %v days on %d total candles\r\n", numberOfRead, len(ts.Candles))
	o.series = ts

	// descending
	bars_len := len(o.series.Candles)
...
func (o *IchimokuIndicator) AnalyseTriggerCross(previous *IchimokuStatus, _52_bars_latest []*techan.Candle) (*IchimokuStatus, error) {
	...
	if !newIchi.bar.Period.Start.After(previous.bar.Period.Start) {
...
func (o *IchimokuIndicator) PreAnalyseIchimoku(data []*IchimokuStatus) (*IchimokuStatus, error) {
	...
	line1_point_a := NewPoint(decimal.NewFromInt(previous.bar.Period.Start.Unix()), previous.TenkenSen.valLine)
	line1_point_b := NewPoint(decimal.NewFromInt(current.bar.Period.Start.Unix()), current.TenkenSen.valLine)

	line2_point_a := NewPoint(decimal.NewFromInt(previous.bar.Period.Start.Unix()), previous.KijonSen.valLine)
	line2_point_b := NewPoint(decimal.NewFromInt(current.bar.Period.Start.Unix()), current.KijonSen.valLine)
...
func (o *IchimokuIndicator) CrossCheck(previous, newIchi *IchimokuStatus) (EInterSectionStatus, decimal.Decimal) {
	line1_point_a := NewPoint(decimal.NewFromInt(previous.bar.Period.Start.Unix()), previous.TenkenSen.valLine)
	line1_point_b := NewPoint(decimal.NewFromInt(newIchi.bar.Period.Start.Unix()), newIchi.TenkenSen.valLine)

	line2_point_a := NewPoint(decimal.NewFromInt(previous.bar.Period.Start.Unix()), previous.KijonSen.valLine)
	line2_point_b := NewPoint(decimal.NewFromInt(newIchi.bar.Period.Start.Unix()), newIchi.KijonSen.valLine)
...
func (o *IchimokuIndicator) Calc_Cloud_InPast(current int) (Point, Point) {
	...
	index := current + 25
	c := o.Ichimokus[index]
	buff_senco_a := NewPoint(decimal.NewFromInt(c.bar.Period.Start.Unix()/1000), c.SenKoA_Shifted26.valLine)
	buff_senco_b := NewPoint(decimal.NewFromInt(c.bar.Period.Start.Unix()/1000), c.SenKoB_Shifted26.valLine)
